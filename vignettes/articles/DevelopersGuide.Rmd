...
`Title`, `Subtitle`, and `Introduction` support multiple languages. Four languages are supported:
...
`Value` is a numeric array containing the values of the performance indicators included in the `Quilt` Object. `Value` must have dimensions `c(nOM, nMP, nPI)` (or 
optionally dimensions `c(nSim, nOM, nMP, nPI)`):
...
Information for the *Trade-off Plot* is stored in a [Tradeoff Object](../reference/Tradeoff-class.html).
...
The way to do this is to clone the *SlickLibrary* repository, than save your Slick Object to the *Slick_Objects* directory, and submit a pull request so that your change can be incorporated into the *SlickLibrary* repo.
...
Your Slick object will be available immediately once the *SlickLibrary* repository is updated.
